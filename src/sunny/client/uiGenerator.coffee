      ...
      if param.type.isPrimitive() == true
        ...
        for subField in subFields
          if subField.type.isPrimitive() == true
            contentCode += "<div> <input type='radio' class='refParam'  data-param='"+param.type.klasses[0].name+"' name='"+sunnyEvent+"-"+param.name+"' value='"+subField.name+"'> "+param.name+"-"+subField.name+" </div>"
...
@formsView = (form) ->
  ...
  for input in $(form).find('.refParam:checked')
    if refParams[$(input).attr("data-param")]
      refParams[$(input).attr("data-param")].push([$(input).attr("data-param"),$(input).val()])
    else
      refParams[$(input).attr("data-param")] = [$(input).attr("data-param"),$(input).val()]
  paramsObj = Sunny.Meta.events[sunnyEvent].__meta__.fields
  formCode = "<div><h4>"+sunnyEvent+" Event</h4>"
  formCode += "<div {{"+sunnyEvent+" }} class='"+sunnyEvent+"Form form-inline' onkeypress='return event.keyCode != 13;'>  <div class='form-group'>"
  for param in params
    formCode += "<div> "+param+":<input type='text' name='"+param+"' placeholder='Enter Text' {{"+sunnyEvent+"_"+param+"}}/> </div>"

  for refParam in Object.keys(refParams)
    formCode += "<div> Select "+refParams[refParam][0]+": <br> <select name="+param+" {{"+sunnyEvent+"_"+param+"}}> "
    for obj in Sunny.Meta.records[refParams[refParam][0]].all()
      formCode += "<option value='"+obj.id()+"'>"+obj[refParams[refParam][1]]+"</option> "
    formCode += "</select> </div>"

  formCode += "<button class='btn btn-default "+sunnyEvent+"' {{sunny_trigger}}> "+sunnyEvent+" </button> </div>"
  ...
  for obj in objects
    ...
    for attr in attributes
      if typeof obj[attr] == "string"
        listCode +="<div sunny-editable-fld='true' sunny-sig='"+model+"' sunny-atom-id='"+obj.id()+"' sunny-fld-name='"+attr+"' class='editable editable-click'>"+attr+": "+obj[attr]+"</div>"
      else if obj[attr] instanceof Array
        for subObj in obj[attr]
          listCode += "<div>"+attr+": "+subObj+"</div>"
      ...
      if obj[subAttr] instanceof Array
        ...
        for subObj in obj[subAttr]
          for subSubAttr in subAttributes[subAttr]
            listCode +="<div sunny-editable-fld='true' sunny-sig='"+subModel+"' sunny-atom-id='"+subObj.id()+"' sunny-fld-name='"+subSubAttr+"' class='editable editable-click'>"+subAttr+"-"+subSubAttr+": "+subObj[subSubAttr]+"</div>"
      else 
        subModel = obj[subAttr].type()
        for subSubAttr in subAttributes[subAttr]
          listCode +="<div sunny-editable-fld='true' sunny-sig='"+subModel+"' sunny-atom-id='"+obj[subAttr].id()+"' sunny-fld-name='"+subSubAttr+"' class='editable editable-click'>"+subAttr+"-"+subSubAttr+": "+obj[subAttr][subSubAttr]+"</div>"
      listCode += "</div>"
...
@generateFormsViewSunnyCode = (form) ->
  ...
  for refParam in Object.keys(refParams)
    formCode += "Select "+refParams[refParam][0]+": <br> <select name="+param+" {{"+sunnyEvent+"_"+param+"}}> \n\ "
    formCode += "{{each "+refParams[refParam][0]+"}} \n\ "
...
@generateListViewSunnyCode = (form) ->
  ...
  sunnyCode = "<div> \n\ {{#each "+model+"}} \n\ <span class='"+model+"-exit pull-right btn btn-default btn-sm my-icon-btn glyphicon glyphicon-remove' {{sunny_trigger event='Destroy' obj=this}}> </span> \n\ "
  for attr in attributes
    if fieldsDic[attr].type.isPrimitive() == true and fieldsDic[attr].type.isComposition() == false
      sunnyCode +="<div class='"+model+"-"+attr+"' ><div {{editableField obj=this fld='"+attr+"'}}> "+attr+" : {{this."+attr+"}}</div></div> <hr> \n\ " 
    else if fieldsDic[attr].type.isPrimitive() == true
      sunnyCode +="<div class='"+model+"-"+attr+"' ><div> "+attr+" : {{this."+attr+"}}</div></div> <hr> \n\ " 
  for subAttr in Object.keys(subAttributes)
    if fieldsDic[subAttr].type.isReference() == true and fieldsDic[subAttr].type.isComposition() == false
      for subSubAttr in subAttributes[subAttr]
        sunnyCode +="<div class='"+model+"-"+subAttr+"-"+subSubAttr+"' ><div {{editableField obj=this fld='"+subSubAttr+"'}}> "+subAttr+"-"+subSubAttr+" : {{this."+subAttr+"."+subSubAttr+"}}</div></div> <hr> \n\ " 
    else if fieldsDic[subAttr].type.isReference() == true and fieldsDic[subAttr].type.isComposition() == true
        sunnyCode += "{{#each this."+subAttr+"}} \n\ "
        for subSubAttr in subAttributes[subAttr]
          sunnyCode +="<div class='"+model+"-"+subAttr+"-"+subSubAttr+"' ><div {{editableField obj=this fld='"+subSubAttr+"'}}> "+subAttr+"-"+subSubAttr+" : {{this."+subSubAttr+"}}</div></div> <hr> \n\ " 
        sunnyCode += "{{/each}}"
